...
#include "Packages/jp.lilxyzw.shadercore/ShaderLibrary/urp_lighting.hlsl"
...
half4 frag(v2f i, bool isFront : SV_IsFrontFace) : SV_Target
{
    ...
    #ifdef NT_FUR
    sd.add += saturate(i.customV2f.furVector.z) * pow(saturate(1-abs(dot(normalize(sd.N), vertex.V))), _FurRimFresnelPower) * lerp(1, saturate(1-sd.lightColor), _FurRimAntiLight) * _FurRimColor.rgb;
    #endif
    ...
    sd.col.rgb = MixFog(sd.col.rgb, InitializeInputDataFog(float4(vertex.position, 1.0), i.fogFactor));
    return sd.col;
}
